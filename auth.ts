...
/**
 * Lazy config so Vercel/runtime env vars (AUTH_SECRET, AUTH_GOOGLE_*) are read when handlers run,
 * not only when the module was first evaluated during `next build`.
 */
  ...
  return {
    ...
    callbacks: {
      authorized({ request, auth: session }) {
        ...
        if (
          mode === "client" &&
          (path.startsWith("/admin") || path === "/dashboard")
        ) {
          if (isAdminRoleStr(role)) {
            return NextResponse.redirect(new URL("/portal", request.nextUrl));
          }
      ...
      async session({ session, token }) {
        ...
        session.user.clientAccountId =
          (token.clientAccountId as string | null) ?? null;
